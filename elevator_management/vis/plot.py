...
num_elev = 3 #TODO set dynamicly. somhow iam too stupit
X_OFFSET = (WINDOW_WIDTH / 2) - num_elev * (ELEVATOR_WIDTH + X_GAP) * 0.5 - 10
...
class Visualizer:
    ...
    def update(self, observations, action):
        ...
        for e in self.elevators_plots:
            if action is None :
                        continue
            if(observations["doors_state"][e.number] == 0 and action["target"][e.number] != observations["position"][e.number]):
                font = pygame.font.Font('freesansbold.ttf', 12)
                t = int(action["target"][e.number])
                text = font.render(str(t), True, LIGHT)
                self.window.blit(text,(e.body.rect.x + ELEVATOR_WIDTH*0.5 - 3, e.body.rect.y+ELEVATOR_HEIGHT*0.5 - 6))
    ...
    def draw_surrounding(self, observations):
        ...
        #draw gras
        pygame.draw.polygon(self.window, (97,184,118), [(0,WINDOW_HEIGHT), (0,GROUND_FLOOR), (WINDOW_WIDTH,GROUND_FLOOR), (WINDOW_WIDTH,WINDOW_HEIGHT)])
        #draw sun
        ...
        for floor_num in range(0, self.num_floors):
            ...
            self.window.blit(text, (house_x - 15, (GROUND_FLOOR - floor_num*FLOOR_SIZE)-(FLOOR_SIZE/2)))
        ...
        for floor_number, call in enumerate(calls):
            ...
            if call[1] == 1:
                c2 = LIGHT
            pygame.draw.polygon(self.window, c2, [(a+DISTANCE_BETWEEN_BUTTONS, b-h), (a+DISTANCE_BETWEEN_BUTTONS+g, b-h), (a+DISTANCE_BETWEEN_BUTTONS+(g/2), b)])
...
class Elevator():
    ...
    def update_observation(self, observation):
        ...
        pygame.draw.line(self.window, DARK_GRAY, (self.body.rect.x+(ELEVATOR_WIDTH*0.5), self.body.rect.y), (self.body.rect.x+(ELEVATOR_WIDTH*0.5), GROUND_FLOOR-self.num_floors*FLOOR_SIZE), 2)
...
class ElevatorBody(pygame.sprite.Sprite):
    def __init__(self, number, observation):
        ...
        x = number * (ELEVATOR_WIDTH + X_GAP) + X_OFFSET
    ...
    def update(self, observation):
        ...
        y = GROUND_FLOOR - FLOOR_SIZE * observation["position"][self.number] - ELEVATOR_HEIGHT
        self.rect.y = y

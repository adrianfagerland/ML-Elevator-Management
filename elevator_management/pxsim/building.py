...
class Building:
    def __init__(
        ...
        density: float,
        seed: int,
        num_of_people_per_floor_per_elevator: int = NUM_OF_PEOPLE_PER_FLOOR_PER_ELEVATOR,
        interfloor_probability: float = INTERFLOOR_PROBABILITY,
        people_at_ground_floor: int = PEOPLE_AT_GROUND_FLOOR,
    ...
    def get_next_arrivals(self, time: datetime):
        ...
        while len(arrivals) == 0:
            ...
            for floor in range(self.num_of_floors + 1):
                while self.r.random() <= (
                    self.density
                    * self.probability_of_arrival_per_floor_per_person

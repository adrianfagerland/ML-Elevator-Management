...
def test_time_simple():
    compare_list = {
        0: [Elevator.Trajectory(0, 0, 0),
            ...
            Elevator.Trajectory(10, 0, DOOR_OPENING_TIME,
                                doors_open=1, doors_open_direction=1),
...
def test_doors_simple():
    ...
    assert pytest.approx(
        total_time - 0.1*DOOR_OPENING_TIME) == test_elevator.get_time_to_target()
    assert pytest.approx(test_elevator.get_doors_open()) == 0.9

    test_elevator.advance_simulation(0.5 * DOOR_OPENING_TIME)
    assert pytest.approx(
        total_time - 0.6 * DOOR_OPENING_TIME) == test_elevator.get_time_to_target()
    ...
    assert pytest.approx(0.4 * DOOR_OPENING_TIME +
                         DOOR_STAYING_OPEN_TIME) == test_elevator.get_time_to_target()
    assert pytest.approx(test_elevator.get_doors_open()) == 0.6

    test_elevator.advance_simulation(0.1 * DOOR_OPENING_TIME)
    assert pytest.approx(0.3 * DOOR_OPENING_TIME +
                         DOOR_STAYING_OPEN_TIME) == test_elevator.get_time_to_target()

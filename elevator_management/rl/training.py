...
def train():
    ...
    env = gym.make(
        env_name, num_floors=10, num_elevators=3, num_arrivals=100, observation_type="discrete", action_type="discrete"
    )
    ...
    #
    ppo_agent = PPO(
        policy_net, state_dim, action_dim, action_view, lr_actor, lr_critic, gamma, K_epochs, eps_clip, info
    )
    ...
    while time_step <= max_training_timesteps:
        ...
        while not done:
            ...
            if time_step % log_freq == 0:
                ...
                print(
                    f"Num of steps {time_step}. With  {sps:.2f} Steps per sec and Total Training time {run_time:.2f}s"
                )
        ...
        if log_running_episodes < 6:
            ...
            averaged_ep_reward = (
                current_ep_reward * avg_reward_update_per + (1 - avg_reward_update_per) * averaged_ep_reward
            )
        log_running_reward += current_ep_reward
        log_running_episodes += 1

        print(
            f"#Epi {i_episode} rew {current_ep_reward:.2f} ave_rew {averaged_ep_reward:.2f} walk {info['num_walked_stairs']} arr {info['num_people_arrived']}"
        )

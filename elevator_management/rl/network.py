...
class AlphaNetwork(ActorCritic):
    def __init__(self, state_dim, action_dim, action_view, options = {}):
        ...
        self.preprocess = nn.Sequential(nn.Linear(self.input_size, 128),
                                        nn.LeakyReLU(0.2),
                                        nn.Linear(128,self.comm_size_in),
                                        nn.Tanh())
        ...
        self.post_actor = nn.Sequential(nn.Linear(self.comm_size_out * self.comm_layers + self.comm_size_in, 128),
                                        nn.LeakyReLU(0.2),
                                        nn.Linear(128,self.elevator_action_dim))
        
        self.post_critic = nn.Sequential(nn.Linear(self.comm_size_out * self.comm_layers + self.comm_size_in, 128),
                                         nn.LeakyReLU(0.2),
                                         nn.Linear(128,1))
    ...
    def actor_parameters(self):
        return list(self.preprocess.parameters()) + list(self.comm_actor.parameters()) + list(self.post_actor.parameters())
    def critic_parameters(self):
        return list(self.comm_critic.parameters()) + list(self.post_critic.parameters())